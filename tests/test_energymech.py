import unittest
from ircsh.backends import EnergyMechBackend
...
class EnergyMechTests(unittest.TestCase):
    def test_identity_and_unit(self):
        b=EnergyMechBackend("guard",FakeRuntime())
        self.assertEqual(ServiceKind.BOT,b.service_id.kind)
        self.assertEqual("energymech",b.backend)
        self.assertEqual("ircsh-energymech-guard.service",b.unit)
    ...
    def test_unsafe_name_rejected(self):
        with self.assertRaises(ValueError):EnergyMechBackend("../../evil",FakeRuntime())
