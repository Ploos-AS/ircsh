...
class HostInspectorTests(unittest.TestCase):
 @patch("ircsh.host_inspector.spwd.getspnam",return_value=SimpleNamespace(sp_pwdp="!$6$locked"))
 ...
 def test_inspects_account_and_lock_state(self,p,g,s):
  a=HostInspector().inspect("alice");self.assertEqual("ircsh",a.group);self.assertEqual("/usr/bin/ircsh",a.shell);self.assertTrue(a.locked)
 @patch("ircsh.host_inspector.spwd.getspnam",return_value=SimpleNamespace(sp_pwdp="$6$active"))
 @patch("ircsh.host_inspector.grp.getgrgid",return_value=SimpleNamespace(gr_name="ircsh"))
 @patch("ircsh.host_inspector.pwd.getpwnam",return_value=SimpleNamespace(pw_uid=1001,pw_gid=1001,pw_dir="/home/alice",pw_shell="/usr/bin/ircsh"))
 def test_unlocked_account(self,p,g,s):self.assertFalse(HostInspector().inspect("alice").locked)
 ...
 @patch("ircsh.host_inspector.spwd.getspnam",side_effect=PermissionError)
 @patch("ircsh.host_inspector.grp.getgrgid",return_value=SimpleNamespace(gr_name="ircsh"))
 @patch("ircsh.host_inspector.pwd.getpwnam",return_value=SimpleNamespace(pw_uid=1001,pw_gid=1001,pw_dir="/home/alice",pw_shell="/usr/bin/ircsh"))
 def test_shadow_permission_failure_fails_closed(self,p,g,s):
  with self.assertRaises(RuntimeError):HostInspector().inspect("alice")
