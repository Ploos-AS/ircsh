...
class DriftTests(unittest.TestCase):
 def test_compliant(self):
  h=HostAccount("alice",1001,1001,"ircsh","/home/alice","/usr/bin/ircsh",False);self.assertEqual((),AccountReconciler().plan(ManagedAccount("alice","irc-basic"),h))
 def test_drift(self):
  h=HostAccount("alice",1001,1001,"users","/srv/alice","/bin/bash",False)
 ...
 def test_disabled_idempotent(self):
  h=HostAccount("alice",1001,1001,"ircsh","/home/alice","/usr/sbin/nologin",True);self.assertEqual((),AccountReconciler().plan(ManagedAccount("alice","irc-basic",False),h))
