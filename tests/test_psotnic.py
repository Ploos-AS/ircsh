import unittest
from ircsh.backends import PsotnicBackend
...
class PsotnicTests(unittest.TestCase):
    def test_identity_and_unit(self):
        b=PsotnicBackend("guard",FakeRuntime())
        self.assertEqual(ServiceKind.BOT,b.service_id.kind)
        self.assertEqual("psotnic",b.backend)
        self.assertEqual("ircsh-psotnic-guard.service",b.unit)
    ...
    def test_unsafe_name_rejected(self):
        with self.assertRaises(ValueError):PsotnicBackend("../../evil",FakeRuntime())
